...
function directBonus(card, board) {
  switch (card.id) {
    ...
    case 'frontHall': return Math.min(2, countIds(board, ['dougongPainting', 'frontCouplet', 'baoshutang', 'swallowTail']));
    case 'courtyard': return Math.min(3, Math.max(0, countType(board, 'space') - 1));
    ...
    case 'ritualHall': return Math.min(2, countType(board, 'text'));
...
const statistics = culturalCards.map((card) => {
  ...
  for (let sample = 0; sample < samplesPerCard; sample += 1) {
    const others = sampleOthers(culturalCards, card.id, 7);
