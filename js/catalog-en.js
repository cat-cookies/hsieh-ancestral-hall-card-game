(() => {
  ...
  function removeSourceAttributions(root = document) {
    ...
    root.querySelectorAll(".catalog-card-body *").forEach((node) => {
      ...
      if (/^(Content basis|Data basis|Source)\s*[：:]/.test(text)) {
        node.remove();
      }
    });
  }
  ...
  function render() {
    ...
    count.textContent = `Showing ${cards.length} of ${DATA.cards.length}`;

    if (!cards.length) {
      grid.innerHTML = '<p class="catalog-empty">No matching cards. Try another keyword or filter.</p>';
      return;
    }
      ...
      return `
        <details class="catalog-card catalog-${escapeHtml(card.type)}">
          <summary>
            ...
            <span class="catalog-card-power" title="${card.type === "effect" ? "Immediate effect" : "Base power"}">${card.type === "effect" ? "✦" : escapeHtml(card.power)}</span>
          </summary>
          <div class="catalog-card-body">
            ...
            <section>
              ...
              <p>${escapeHtml(card.valueNote || "This card presents an important aspect of the hall’s history, space, craftsmanship, or ritual order.")}</p>
            </section>
          </div>
        </details>`;
    }).join("");

    // 卡牌圖鑑只呈現Game Effect、Explanation與Heritage Value；不顯示逐卡內容依據。
